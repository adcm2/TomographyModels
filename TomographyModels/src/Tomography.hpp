...
public:
    ...
    Tomography(const std::string &nc_filename)
    {
        ...
        if (retval != 0)
            throw std::runtime_error(nc_strerror(retval));
        for (std::size_t i = 0; i < depth_len; ++i)
            depth[i] = data[i];
        delete[] data;
        ...
        if (retval != 0)
            throw std::runtime_error(nc_strerror(retval));
        for (std::size_t i = 0; i < lat_len; ++i)
            lat[i] = data[i];
        delete[] data;
        ...
        if (retval != 0)
            throw std::runtime_error(nc_strerror(retval));
        for (std::size_t i = 0; i < lon_len; ++i)
            lon[i] = data[i];
        delete[] data;
...
double Tomography::GetValueAt(const double &d, double lo, const double &la, const bool &outofBound) const
{
    ...
    if (la < lat[0] || la > lat.back())
        return 0.0 / 0.0;
    if ((d <= depth[0] || d >= depth.back()) && !outofBound)
        return 0.0 / 0.0;
    if (d <= depth[0] && outofBound)
        return GetValueAt(depth[0] + 1e-5, lo, la);
    if (d >= depth.back() && outofBound)
        return GetValueAt(depth.back() - 1e-5, lo, la);
    ...
    if (index_prev_lon > index_lon)
        pp[0] = ((lo < lon[index_prev_lon] ? lo + 360 : lo) - lon[index_prev_lon]) / (lon[index_lon] + 360 - lon[index_prev_lon]);
    else
